...
class Detect(nn.Module):
    ...
    def __init__(self, nc=80, ch=()):  # detection layer
        ...
        self.cv2 = nn.ModuleList(nn.Sequential(Conv(x, c2, 3), Conv(c2, c2, 3)) for x in ch)
        self.cv3 = nn.ModuleList(nn.Sequential(Conv(x, c3, 3), Conv(c3, c3, 3)) for x in ch)
        self.cv_out = nn.ModuleList(nn.Conv2d(c2 + c3, self.no, 1) for _ in ch)
    ...
    def forward(self, x):
        ...
        up_x1 = x[1] + self.upconv1(x[2])
        up_x0 = x[0] + self.upconv2(x[1])
        x = [up_x0, up_x1, x[2]]
        for i in range(self.nl):
            x[i] = self.cv_out[i](x[i])
